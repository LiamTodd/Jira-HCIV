...
function App() {
  ...
  useEffect(() => {
    if (originalPredictionData !== null) {
      if (fromDate || toDate || filterStatus || filterPriority) {
        let filteredPredictionData = originalPredictionData;

        if (fromDate || toDate) {
          filteredPredictionData = filteredPredictionData
            .filter((issue) => {
              if (fromDate) {
                return (
        ...
        if (filterStatus.length > 0) {
          filteredPredictionData = filteredPredictionData.filter((issue) =>
            filterStatus.includes(issue.status)
          );
        }
        if (filterPriority.length > 0) {
          filteredPredictionData = filteredPredictionData.filter((issue) =>
            filterPriority.includes(issue.priority)
          );
        }
  ...
  }, [fromDate, toDate, originalPredictionData, filterStatus, filterPriority]);
